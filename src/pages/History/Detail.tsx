...
const statusConfig: Record<string, { label: string; color: string; bgColor: string; icon: React.ReactNode; step: number }> = {
  ...
  [ORDER_STATUS.CONFIRMED]: { label: 'Đã xác nhận', color: 'text-blue-600', bgColor: 'bg-blue-50', icon: <CheckCircle className="w-5 h-5" />, step: 2 },
  [ORDER_STATUS.PROCESSING]: { label: 'Đang xử lý', color: 'text-cyan-600', bgColor: 'bg-cyan-50', icon: <Package className="w-5 h-5" />, step: 3 },
  [ORDER_STATUS.DELIVERED]: { label: 'Đã giao', color: 'text-emerald-600', bgColor: 'bg-emerald-50', icon: <Truck className="w-5 h-5" />, step: 4 },
  [ORDER_STATUS.CANCELLED]: { label: 'Đã hủy', color: 'text-rose-600', bgColor: 'bg-rose-50', icon: <X className="w-5 h-5" />, step: 0 },
};
        ...
        <section className="w-full bg-gradient-to-b from-gray-50 to-white py-4 md:py-8 lg:py-16">
          <div className="container mx-auto max-w-4xl px-3 sm:px-4 md:px-6">
            ...
            <motion.div 
              ...
              animate={{ opacity: 1 }}
            >
              <p className="text-red-600 font-semibold">{error || 'Không tìm thấy đơn hàng'}</p>
            </motion.div>
          </div>
        </section>
      </Layout>
    ...
    <Layout>
      <section className="w-full bg-gradient-to-b from-gray-50 to-white py-4 md:py-8 lg:py-16">
        <div className="container mx-auto max-w-4xl px-3 sm:px-4 md:px-6">
          ...
          {/* Order Header */}
          <motion.div
            className="mb-6 md:mb-8 bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden"
          ...
          >
            ...
            <div className="md:hidden px-3 py-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex-shrink-0">
                  <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${orderStatus.bgColor} ${orderStatus.color}`}>
                    {orderStatus.icon}
            ...
            <div className="hidden md:flex md:items-center md:justify-between md:gap-6 p-6">
              ...
              <div className="flex-1">
                ...
                <p className="font-semibold text-lg text-gray-900">{order.id.slice(0, 12).toUpperCase()}</p>
              </div>
              
              {/* Status Badge */}
              <div className="flex-1 text-center">
                <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold ${orderStatus.bgColor} ${orderStatus.color}`}>
                  {orderStatus.icon}
              ...
              <div className="flex-1 text-right">
                ...
                <p className="font-semibold text-lg text-gray-900">{createdDate}</p>
              </div>
            </div>
          </motion.div>
          ...
          {order.shippingAddress && (
            <motion.div
              className="mb-4 md:mb-6 bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-5"
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.05 }}
            >
              <h2 className="text-sm md:text-lg font-bold text-gray-900 mb-2.5 flex items-center gap-2">
                <MapPin className="w-4 h-4 md:w-5 md:h-5 flex-shrink-0" />
                Địa chỉ giao hàng
              </h2>
                <p className="text-sm md:text-sm text-gray-900 whitespace-pre-wrap break-words leading-relaxed">
                  {order.shippingAddress}
                </p>
            </motion.div>
          )}
          ...
          {!isCancelled && (
            <motion.div
              className="mb-6 md:mb-8 bg-white rounded-lg shadow-sm border border-gray-200 p-3 md:p-6"
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.15 }}
            >
              <h2 className="text-base md:text-xl font-bold text-gray-900 mb-6 md:mb-8">Quá trình xử lý đơn hàng</h2>
              
              {/* Horizontal Timeline */}
              <div className="relative py-2 md:py-4">
                {/* Timeline line */}
                <div className="absolute top-4 md:top-6 left-3 right-3 md:left-6 md:right-6 h-0.5 bg-gray-200" />
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-center">
                          <p className={`font-semibold text-xs md:text-sm leading-tight ${isCompleted ? 'text-gray-900' : 'text-gray-500'}`}>
                            {timelineStep.label}
                          </p>
                          {isCompleted && (
              ...
              <div className="border-t border-gray-100 p-4 md:p-6">
                ...
                <div className="space-y-3 border-b border-gray-100 pb-4 mb-4">
                  ...
                  {order.discountAmount && order.discountAmount > 0 && (
                    <motion.div
                      className="flex justify-between text-sm bg-emerald-50 -mx-2 px-2 py-2 rounded"
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      transition={{ delay: 0.5 }}
                    >
                      <p className="text-emerald-700">Khuyến mãi</p>
                      <p className="font-semibold text-emerald-700">-{formatPrice(order.discountAmount)}</p>
                    </motion.div>
                  )}
                </div>

                <div className="flex justify-between items-center">
                  <p className="font-bold text-gray-900 text-base md:text-lg">Tổng cộng</p>
